...
class ReportSaleDetails(models.AbstractModel):
    ...
    def get_sale_details(self, date_start=False, date_stop=False, config_ids=False, session_ids=False):
        ...
        # tax_today = self.env['res.currency.rate'].search([('name', 'in', date_start)],limit=1)
        ...
        return {
            ...
            'tax_today': self.get_tax_today(session_ids),
        }
    ...
    def get_tax_today(self, session_ids):
        sessions = self.env['pos.session'].browse(session_ids)
        tax_today = sessions[0].tax_today
        print("Valor de tax_today:", tax_today)
        return tax_today
    ...
    class PosSession(models.Model):
        ...
        def _get_invoice_total_list(self):
            ...
            for order in self.order_ids.filtered(lambda o: o.state != 'draft'):
                invoice_id = self.env['account.move'].search([('ref', '=', order.name)],limit=1)
                invoice = {
                    'total': order.amount_total,
                    'name': order.name,
